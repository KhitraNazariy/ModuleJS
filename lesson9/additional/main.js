...
for (const user of usersList) {
    ...
    for (const userKey in user) {
        if (typeof user[userKey] !== 'object') {
            ...
            aboutUser.innerText = `${userKey} - ${user[userKey]}`;
            divUser.append(aboutUser);
        } else if (userKey === 'address') {
            let address = document.createElement('div');
            for (const key in user[userKey]) {
                if (typeof user[userKey][key] !== 'object') {
                    let addressDiv = document.createElement('div');
                    addressDiv.innerText = `${key} - ${user[userKey][key]}`;
                    address.append(addressDiv);
                } else if (typeof user[userKey][key] === 'object') {
                    let geo = document.createElement('div')
                    for (const keyGeo in user[userKey][key]) {
                        let geoDiv = document.createElement('div');
                        geoDiv.innerText = `${keyGeo} - ${user[userKey][key][keyGeo]}`;
                        geo.appendChild(geoDiv);
                    }
            ...
            for (const keyCompany in user[userKey]) {
                let divCompany = document.createElement('div');
                divCompany.innerText = `${keyCompany} - ${user[userKey][keyCompany]}`;
                company.appendChild(divCompany);
            }
            divUser.appendChild(company)
        }
    }
